...
async def _scan(client: AsyncClient, prompt: str) -> dict:
...
class TestBenignFlow:
    async def test_benign_full_flow(self, client: AsyncClient) -> None:
        item = await _scan(client, BENIGN)

        # Decision
        assert item["decision"].upper() == "ALLOW"

        # Storage: retrievable by ID
        detail = await client.get(f"/api/v1/analysis/{item['analysis_id']}")
        assert detail.status_code == 200
        assert detail.json()["data"]["analysis_id"] == item["analysis_id"]

        # History contains it, most recent first
        history = await client.get("/api/v1/analysis?limit=20")
...
class TestSuspiciousFlow:
    async def test_suspicious_flags_for_review(self, client: AsyncClient) -> None:
        item = await _scan(client, SUSPICIOUS)
...
class TestMaliciousFlow:
    async def test_malicious_blocked_and_auditable(self, client: AsyncClient) -> None:
        item = await _scan(client, MALICIOUS)
    ...
    async def test_attack_variants_never_allowed(self, client: AsyncClient, attack: str) -> None:
        item = await _scan(client, attack)
...
class TestSummaryConsistency:
    async def test_summary_counters_match_history(self, client: AsyncClient) -> None:
        """After a fixed scan sequence, summary counts must match reality."""
        expected_decisions = []
        for prompt in (BENIGN, MALICIOUS, SUSPICIOUS):
            item = await _scan(client, prompt)
            expected_decisions.append(item["decision"].lower())

        summary_resp = await client.get("/api/v1/console/summary")
        assert summary_resp.status_code == 200

        summary = summary_resp.json()["data"]
        history_resp = await client.get("/api/v1/analysis?limit=200")
    ...
    async def test_repeated_scan_stable_results(self, client: AsyncClient) -> None:
        decisions = [(await _scan(client, MALICIOUS))["decision"] for _ in range(3)]
