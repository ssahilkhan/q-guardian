...
| `[project]` | `name = "q-guardian"`, `version = "1.1.0"`, description *"Q-Guardian: A Hybrid Quantum-Classical Framework for Runtime Security of Autonomous AI Agents"*, `readme = "README.md"`, `license = MIT`, `requires-python = ">=3.12"`, author "Q-Guardian Research Team", keywords, classifiers (Beta, Python 3.12/3.13, MIT, Security/AI) |
...
> **Note on README test count:** the README badge and the `05_Test_File_Documentation.md` audit are in sync at **2,650 test functions** (current CI run).
...
- `PKG-INFO` — distribution metadata (name `q-guardian`, version `1.1.0`, description, URLs, classifiers, `Requires-Python >=3.12`, full deps + extras).
