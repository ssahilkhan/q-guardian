...
class MLConfig(BaseModel):
    ...
    max_features: int = Field(
        default=10_000, description="Maximum vocabulary size for TF-IDF"
    )
    ...
    default_cv_folds: int = Field(
        default=5, ge=2, description="Default cross-validation folds"
    )
    random_state: int = Field(default=42, description="Random seed for reproducibility")

    # XGBoost (optional)
    xgboost_available: bool = Field(
        default=False, description="Whether XGBoost is installed"
    )
