...
def parse_label(raw) -> int | None:
    ...
    if s in ("0", "benign", "safe", "allow", "ok", "good"):
        return 0
    if s in ("1", "malicious", "unsafe", "threat", "escalate", "block", "deny", "bad", "attack"):
        return 1
    return None
    ...
    if path.suffix.lower() in (".jsonl", ".ndjson"):
        ...
        with open(path, encoding="utf-8-sig") as f:
            for row in csv.DictReader(f):
                text = row.get("text") or row.get("prompt") or row.get("statement") or ""
                label = parse_label(row.get("label"))
                if text.strip() and label is not None:
                    samples.append((text.strip(), label))
    else:
        raise SystemExit(
            f"Unsupported file type: {path.suffix} (use .csv, .jsonl, .ndjson or .json)"
        )
    return samples
...
def main() -> None:
    ...
    parser.add_argument(
        "--base", action="store_true", help="also keep the built-in 20-prompt demo corpus"
    )
    parser.add_argument(
        "--replace", action="store_true", help="explicitly replace the built-in corpus (default)"
    )
    parser.add_argument(
        "--max-samples",
        type=int,
    ...
    parser.add_argument(
        "--qsvm-samples", type=int, default=60, help="max samples per class for QSVM (default 60)"
    )
    parser.add_argument(
        "--state-dir", default=str(DEFAULT_STATE_DIR), help="where trained models are saved"
    )
    ...
    print(f"Dataset: {len(raw)} samples ({len(benign)} benign / {len(malicious)} malicious)")
    ...
    if args.max_samples:
        ...
        print(
            f"After capping to {args.max_samples}/class: "
    ...
    if args.qsvm_samples and args.qsvm_samples < len(corpus):
        ...
        qsvm_corpus = qb + qm
        print(f"QSVM will train on a balanced subset: {len(qb)} benign / {len(qm)} malicious")

    print(f"Training on {len(corpus)} samples (counts may include duplicates)...")
    t0 = time.monotonic()
    ...
    y_all = np.array([label for _, label in corpus])
    ...
    async def _self_check() -> float:
        ...
        return float(np.mean([p == y for p, y in zip(rf_pred, y_all, strict=True)]))
    ...
    print(f"RandomForest self-check accuracy on the {len(corpus)} samples: {acc * 100:.1f}%")
